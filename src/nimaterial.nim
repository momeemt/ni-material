...
proc getCSSParameter(keyName: string, baseStyle: string): string {.deprecated.} =
...
proc color*[T](className: T, color: Color) {.deprecated.} =
  let baseStyle = "color: " & $color
  resultCSS[$className] = getCSSParameter($className, baseStyle)

proc text*[T](className: T, font: string = fontFamilyDefault, size: string = "16px", lineHeight: string = "normal", align: Flex = fStart) {.deprecated.} =
...
proc floatShadow*[T](className: T, time: float) {.deprecated.} =
